...
reservoir =                 {'label':   'infinite heatsink',       # this might change if we want locally powered or observed chunks
                             'ref':     '~',
                             'eltype':  'reservoir',     
                             'k':       h_to_k(h=534, axis="z", x=0.0001, y=0.0005, z=0.001),
                             'cp':      1,    
                             'rho':     1}
...
dt = 1 / (frequency * 20)  # Time step, smaller than 1/60 s for smooth curve

#P_of_t = lambda t: 100 if t <= 0.25 else 0
#P_of_t = lambda t: math.sin(2*math.pi*frequency*t) + 1

#randomize P between 0.1 and 1 (in 0.1 increments) every 20 seconds  
P_of_t = lambda t: (((int(t / 20) * 7919) % 10) + 1) / 10  # Changes every 20s, values from 0.1 to 1.0 in 0.1 steps
...
# copper
my_model.create_volume(x_vol=0.003,y_vol=0.010,z_vol=0.000035,x_el=0.0001,y_el=0.0005,z_el=0.000035,properties=null,T=0,P=0)                                      
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=30,y_ext=10,z_ext=1,properties=copper,T=0,P=0)             
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=6,y_ext=10,z_ext=1,properties=null,T=0,P=0)             

# solder
my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.0001,thickness_element=0.0001,properties=null)            
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=30,y_ext=10,z_ext=1,properties=mask,T=0,P=0)             
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=24,y_ext=10,z_ext=1,properties=solder,T=0,P=0)             
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=6,y_ext=10,z_ext=1,properties=null,T=0,P=0)             

# Alumina
my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.00066,thickness_element=0.00011,properties=tim)            
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=14,y_ext=6,z_ext=6,properties=alumina,T=0,P=0)             
my_model.modify_volume(x_coord=1,y_coord=1,z_coord=2,x_ext=10,y_ext=1,z_ext=5,properties=thermistor,T=0,P=0)

# resistor
my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.00002,thickness_element=0.00002,properties=tim)            
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=10,y_ext=6,z_ext=1,properties=resistor,T=0,P=P_of_t)             

# glass
my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.00002,thickness_element=0.00002,properties=tim)            
my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=10,y_ext=6,z_ext=1,properties=glass,T=0,P=0)             

# tim, remainder
my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.000309,thickness_element=0.000103,properties=tim)            

# void
#my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=10,y_ext=6,z_ext=3,properties=null,T=0,P=0)             
...
my_model.observe_these_labels_T_avg([resistor, thermistor, aluminum])
my_model.observe_these_labels_T_max([resistor, solder])
my_model.observe_these_labels_P([resistor])

df = my_model.solve(dt=0.00003,dt_sampling=0.001,t_max=200,dT_dt_converge=0.00000001)
