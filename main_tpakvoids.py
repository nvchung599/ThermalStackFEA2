...
reservoir =                 {'label':   'infinite heatsink',       # this might change if we want locally powered or observed chunks
                             'ref':     '~',
                             'eltype':  'reservoir',     
                             'k':       h_to_k(h=10000, axis="z", x=0.001, y=0.001, z=0.001),
                             'cp':      1,    
                             'rho':     1}

copper_heatsink =           {'label':   'copper_heatsink',
                             'ref':     'C',
                             'eltype':  'solid',
                             'k':       400,
                             'cp':      0.385,    
...
copper_amb_f =              {'label':   'copper_amb_f',
                             'ref':     'C',
                             'eltype':  'solid',
                             'k':       400,
                             'cp':      0.385,    
...
copper_amb_b =              {'label':   'copper_amb_b',
                             'ref':     'C',
                             'eltype':  'solid',
                             'k':       400,
                             'cp':      0.385,    
...
silicon =                   {'label':   'silicon',
                             ...
                             'k':       500,
                             'cp':      0.703,    
                             'rho':     23300}   # actually 2330, mod by 10x for simulation stability
...
solder =                    {'label':   'solder',
                             ...
                             'k':       59,
                             'cp':      0.232,    
                             'rho':     73700}  # actually 7370, mod by 10x for simulation stability

nitride =                   {'label':   'nitride',
                             'ref':     'N',
                             'eltype':  'solid',
                             'k':       321,
                             'cp':      0.8,    
...
for void_size in range (1,2):
    my_model = ThermalModel()
    # reservoir
    my_model.create_volume(x_vol=0.020,y_vol=0.030,z_vol=0.001,x_el=0.001,y_el=0.001,z_el=0.001,properties=reservoir,T=0,P=0)                                       #base, src
    # heatsink
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.003,thickness_element=0.001,properties=copper_heatsink)             
    # solder TIM
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.0004,thickness_element=0.0004,properties=null)             
    my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=14,y_ext=18,z_ext=1,properties=solder,T=0,P=0)             #rod
    #my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=void_size,y_ext=void_size,z_ext=1,properties=null,T=0,P=0)             #rod
    # amb cu b
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.0008,thickness_element=0.0008,properties=null)            
    my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=14,y_ext=18,z_ext=1,properties=copper_amb_b,T=0,P=0)             #rod
    # amb AlN
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.0005,thickness_element=0.0005,properties=null)            
    my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=16,y_ext=20,z_ext=1,properties=nitride,T=0,P=0)             #rod
    # amb cu f
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.0008,thickness_element=0.0008,properties=null)            
    my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=14,y_ext=18,z_ext=1,properties=copper_amb_f,T=0,P=0)             #rod
    # solder TIM
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.00003,thickness_element=0.00003,properties=null)             
    my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=10,y_ext=4,z_ext=1,properties=solder,T=0,P=0)             #rod
    # silicon
    my_model.extend_volume(axis="z",polarity=1,size_extrusion=0.000175,thickness_element=0.000175,properties=null)             
    my_model.modify_volume_centered_slice(axis="z",polarity=-1,index_start_extrusion=-1,x_ext=10,y_ext=4,z_ext=1,properties=silicon,T=0,P=1)             #rod
    
    my_model.visualize
    
    
    my_model.generate_nodes()
    my_model.converge_this_label_T(silicon)
    my_model.observe_these_labels_T_avg([silicon])
    my_model.observe_these_labels_T_max([silicon])
    #my_model.observe_these_labels_P([silicon])

    df = my_model.solve(dt=0.0001,dt_sampling=0.001,t_max=3,dT_dt_converge=0.0001)
...
df = pd.DataFrame({'Column1': void_sizes, 'Column2': T_avg_list, 'Column3':T_max_list})
...
label_to_color = {
    silicon['label']:       "red",
    solder['label']:        "green",
    copper_amb_b['label']:        "orange",
    copper_amb_f['label']:        "orange",
    copper_heatsink['label']:        "orange",
    nitride['label']:       "purple",
